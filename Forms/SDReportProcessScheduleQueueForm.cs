...
{
    public partial class SDReportProcessScheduleQueueForm : Form
    {
        private readonly SDReportProcessScheduleQueueRepository _repository;
        private List<SDReportProcessScheduleQueueModel> _queueData;
        ...
        public SDReportProcessScheduleQueueForm(
    ...
    SamsDeliveryReportForm samsDeliveryForm)
        {
            ...
            _repository = new SDReportProcessScheduleQueueRepository(connectionString);
            _individualStatusChanges = new Dictionary<int, string>();
            _bindingSource = new BindingSource();

            LoadStatusesFromDatabase();

            this.Load += SDReportProcessScheduleQueueForm_Load;
            this.FormClosing += SDReportProcessScheduleQueueForm_FormClosing;
        }
        ...
        private void InitializeComponent()
        {
            ...
            this.menuItemFileType = new ToolStripMenuItem("Bulk File")  // Display current file name
            {
                Font = new Font("Segoe UI", 10F, FontStyle.Bold),
                ForeColor = Color.Black,
            ...
            this.menuItemRawFile.Click += MenuItemRawFile_Click_SD;

            this.menuItemBulkFile = new ToolStripMenuItem();
            this.menuItemBulkFile.Text = "Bulk File";
            this.menuItemBulkFile.Checked = true;  // This is the Bulk File form
            this.menuItemBulkFile.Click += MenuItemBulkFile_Click_SD;

            this.menuItemProcessedFile = new ToolStripMenuItem();
            this.menuItemProcessedFile.Text = "Processed File";
            this.menuItemProcessedFile.Click += MenuItemProcessedFile_Click_SD;
            ...
            this.lblTitle.Text = "SD Report Process Schedule Queue Management";
            ...
            // In InitializeComponent method, replace the DataGridView section:
            ...
            this.Name = "SDReportProcessScheduleQueueForm";
            this.StartPosition = FormStartPosition.CenterScreen;
            this.Text = "SD Report Process Schedule Queue - Status Management (Bulk File)";
        ...
        private async void SDReportProcessScheduleQueueForm_Load(object sender, EventArgs e)
        {
        ...
        private async System.Threading.Tasks.Task LoadQueueDataAsync(bool isInitialLoad = false)
        {
            try
            {
                ...
                {
                    _queueData = _repository.GetAllSDReportProcessScheduleQueue();
                });

                _dataTable = new DataTable();
                _dataTable.Columns.Add("Select", typeof(bool));
                _dataTable.Columns.Add("SdReportScheduleQueueUid", typeof(int));
                _dataTable.Columns.Add("CompanyUid", typeof(int));
                _dataTable.Columns.Add("CompanyName", typeof(string));
                _dataTable.Columns.Add("SdRawReportUid", typeof(int));
                ...
                foreach (var queue in _queueData)
                {
                    _dataTable.Rows.Add(
                        false,
                        queue.SdReportScheduleQueueUid,
                        queue.CompanyUid ?? (object)DBNull.Value,
                        queue.CompanyName ?? "",
                        queue.SdRawReportUid ?? (object)DBNull.Value,
        ...
        private void FormatDataGridView()
        {
            ...
            if (dgvQueue.Columns.Contains("SdReportScheduleQueueUid"))
                dgvQueue.Columns["SdReportScheduleQueueUid"].Visible = false;
            if (dgvQueue.Columns.Contains("CompanyUid"))
                dgvQueue.Columns["CompanyUid"].Visible = false;
            if (dgvQueue.Columns.Contains("SdRawReportUid"))
                dgvQueue.Columns["SdRawReportUid"].Visible = false;
            ...
            dgvQueue.Columns["ProcessedFilePath"].Width = 250;
        ...
        private void DgvQueueCellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (columnName == "Status")
            {
                ...
                int queueId = Convert.ToInt32(row.Cells["SdReportScheduleQueueUid"].Value);
                string newStatusStr = row.Cells["Status"].Value?.ToString() ?? "Pending";

                var originalRecord = _queueData.FirstOrDefault(q => q.SdReportScheduleQueueUid == queueId);
                ...
                if (_dataTable != null)
                {
                    var dataRow = _dataTable.AsEnumerable()
                        .FirstOrDefault(r => r.Field<int>("SdReportScheduleQueueUid") == queueId);
                    if (dataRow != null)
        ...
        private void MenuItemRawFile_Click_SD(object sender, EventArgs e)
        {
        ...
        private void MenuItemBulkFile_Click_SD(object sender, EventArgs e)
        {
            menuItemRawFile.Checked = false;
            menuItemBulkFile.Checked = true;
            menuItemProcessedFile.Checked = false;
        ...
        private void MenuItemProcessedFile_Click_SD(object sender, EventArgs e)
        {
            menuItemRawFile.Checked = false;
            menuItemBulkFile.Checked = false;
            ...
            // ✅ Create new Processed File form if not found
            var processedForm = new ReportProcessScheduleQueueForm(
                _emailConfig,
                _actualR26QueueForm,
                _rawFileForm,
                _samsDeliveryForm,
                this  // Pass this Bulk File form
            );
        ...
        protected override void OnVisibleChanged(EventArgs e)
        {
            ...
            if (this.Visible)
            {
                // Reset menu selection to Bulk File when this form becomes visible
                menuItemRawFile.Checked = false;
                menuItemBulkFile.Checked = true;
                menuItemProcessedFile.Checked = false;
                menuItemFileType.Text = "Bulk File"; // Update parent menu text
                this.Text = "SD Report Process Schedule Queue - Status Management (Bulk File)";
        ...
        private void DgvQueueRowPrePaint(object sender, DataGridViewRowPrePaintEventArgs e)
        {
            ...
            var row = dgvQueue.Rows[e.RowIndex];
            object idValue = row.Cells["SdReportScheduleQueueUid"].Value;
            if (idValue == null)
                return;
        ...
        private void RefreshGridHighlighting()
        {
            ...
            {
                if (row.Cells["SdReportScheduleQueueUid"].Value != null &&
                    int.TryParse(row.Cells["SdReportScheduleQueueUid"].Value.ToString(), out int queueId))
                {
                    if (_individualStatusChanges.ContainsKey(queueId))
                        row.DefaultCellStyle.BackColor = Color.LightYellow;
                    else
        ...
        private void ApplyFilters()
        {
            ...
            {
                filters.Add(
                    $"Convert(SdReportScheduleQueueUid, 'System.String') LIKE '%{searchText}%' OR " +
                    $"CompanyName LIKE '%{searchText}%' OR " +
                    $"RawFilePath LIKE '%{searchText}%'");
            }
        ...
        private void UpdateSelectedRecordsGrid()
        {
            ...
            var selectedRows = _dataTable.AsEnumerable()
                ...
                .Select(r => new
                {
                    ID = r.Field<int>("SdReportScheduleQueueUid"),
                    CompanyName = r.Field<string>("CompanyName"),
                    RawFilePath = r.Field<string>("RawFilePath"),
            ...
            selectedTable.Columns.Add("Raw File Path", typeof(string));
            selectedTable.Columns.Add("Status", typeof(string));

            foreach (var item in selectedRows)
                selectedTable.Rows.Add(item.CompanyName, item.RawFilePath, item.Status);
        ...
        {
            // Same behaviour as RpaScheduleQueueDetailForm "Menu" – just close and go back.
            _isExiting = true;
            this.Close();
        }
        ...
        {
            // Stay on this form; this is already the raw file view.
        }

        private void SDReportProcessScheduleQueueForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (_isExiting)
                return;
